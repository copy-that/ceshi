    ...
    <div class="navlist">
      <div class="navlist_top">
        <div v-for="(item,key) in list" v-bind:key="key">
          <ul
            v-if="item.valueField!=='zd_xinxi'&&item.valueField!=='zd_fenzhan'&&item.valueField!=='zd_mianji'&&item.valueField!=='zd_buy_shangpu'"
          >
            <li class="title">{{item.valueName}}</li>
            <li
              class="sort"
              :class="res.id==label[key]?'active':''"
              v-for="(res,index) in item.zdValue"
              v-bind:key="index"
              @click="changeList(key,res.id)"
              :id="res.id"
            >{{res.value}}</li>
          </ul>
          <ul v-if="item.valueField=='zd_buy_shangpu'">
            <li class="title">{{item.valueName}}</li>
          ...
          <ul class="nav_bot" v-if="item.valueField=='zd_fenzhan'">
            ...
            <li class="placename" :class="tabbar?'is_active':''">
              <span
                v-for="(res,index) in item.zdValue"
                v-bind:key="index"
                :class="res.id==label[key]?'activeColor':''"
                @click="changeList(key,res.id)"
                :id="res.id"
              >{{res.value}}</span>
            </li>
      ...
      <div class="navlist_bot">
        ...
        <div class="infolist" v-for="(item,index) in infolist" v-bind:key="index">
          <router-link  tag="a" target="_blank" :to="{ name: 'navlistXq', query: { id: item.id }}">
            ...
            <ul class="money" style="width:260px;text-align: center">
...
export default {
  ...
  methods: {
    ...
    changeList(key, id) {
      ...
      this.$http("/app/rendInfoApi/getShopList", "post", data, "").then(res => {
        if (res.data.code == 100) {
          this.numbars = res.data.data.rendNum;
          this.totalCount =
            Math.ceil(res.data.data.rendNum / this.pagesize) * 10;
          for (var i = 0; i < res.data.data.rendVos.length; i++) {
            res.data.data.rendVos[i].picUrls = res.data.data.rendVos[
              i
            ].picUrls.split(",");
          }
