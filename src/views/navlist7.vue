        ...
        <div class="navlist">
            <div class="navlist_top">
                <div v-for="(item,key) in list" v-bind:key="key">
                    <ul v-if="item.valueField!=='zd_xinxi'&&item.valueField!=='zd_fenzhan'&&item.valueField!=='zd_mianji'&&item.valueField!=='zd_zu_shangpu'">
                    ...
                    <ul v-if="item.valueField=='zd_zu_shangpu'">
                        <li class="title">{{item.valueName}}</li>
...
export default {
  ...
  methods: {
    ...
    changeList(key,id){
        ...
        this.$http('/app/rendInfoApi/getShopList','post',data,'').then(
            res=>{
   ...
   created(){
    ...
    //      this.label[3]=''
    //  }else{
    //     this.label[3]=this.$store.state.cityId
    //  }
    // 数据字典
    var data=this.qs.stringify({
        type:4,
