        ...
        <div class="navlist">
            <div class="navlist_top">
                <div v-for="(item,key) in list" v-bind:key="key">
                    <ul v-if="item.valueField!=='zd_xinxi'&&item.valueField!=='zd_fenzhan'&&item.valueField!=='zd_zu_zhuzhai'&&item.valueField!=='zd_mianji'">
                        <li class="title">{{item.valueName}}</li>
                        <div style="float:left;width:1100px;">
                            <li class="sort" :class="res.id==label[key]?'active':''"  v-for="(res,index) in item.zdValue"  v-bind:key="index" @click="changeList(key,res.id)" :id="res.id">{{res.value}}</li>
                        </div>
                    </ul>
                    <ul v-if="item.valueField=='zd_zu_zhuzhai'">
                        <li class="title">{{item.valueName}}</li>
                        <div style="float:left;width:1100px;">
                            <li class="sort" :class="res.id==label[key]?'active':''"  v-for="(res,index) in item.zdValue"  v-bind:key="index" @click="changeList(key,res.id)" :id="res.id">
                                ...
                                <!-- <span v-else>{{res.value}}元/月</span> -->
                                <span v-else-if="res.valueType==0">{{res.value}}元/月以下</span>
                                <span v-else-if="res.valueType==1">{{res.value}}元/月</span>
                                <span v-else-if="res.valueType==2">{{res.value}}元/月以上</span>
                            </li>
                        </div>
                    </ul>
            ...
            <div class="navlist_bot">
                ...
                <div class="infolist" v-for="(item,index) in infolist" v-bind:key="index">
                    ...
                    <ul class="money" style="width:260px;text-align: center"><span>{{item.price }}</span>元/月</ul></router-link>
                </div>
                <el-pagination
                    background
  ...
  methods: {
    ...
    changeList(key,id){
        ...
        var data=this.qs.stringify({
            ...
            form:this.label[2],
            dicorationNum:this.label[3],
            classify:this.label[4],
            price:this.label[5],
            area:this.label[6],
        ...
        this.$http('/app/rendInfoApi/getRendHouseList','post',data,'').then(
            res=>{
   ...
   created(){
    ...
    //      this.label[7]=''
    //  }else{
    //     this.label[7]=this.$store.state.cityId
    //  }
    // 数据字典
    var data=this.qs.stringify({
        type:2,
