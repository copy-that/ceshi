        ...
        <div class="navlist">
            <div class="navlist_top">
                <div v-for="(item,key) in list" v-bind:key="key">
                    <ul v-if="item.valueField!=='zd_xinxi'&&item.valueField!=='zd_fenzhan'&&item.valueField!=='zd_mianji'&&item.valueField!=='zd_workshop_zj'">
                    ...
                    <ul v-if="item.valueField=='zd_mianji'">
                     ...
                     <ul v-if="item.valueField=='zd_workshop_zj'">
                        <li class="title">{{item.valueName}}</li>
...
export default {
  ...
  methods: {
    ...
    changeList(key,id){
        ...
        this.$http('/app/rendInfoApi/getWorkshopList','post',data,'').then(
            res=>{
   ...
   created(){
    ...
    //      this.label[4]=''
    //  }else{
    //     this.label[4]=this.$store.state.cityId
    //  }
    // 数据字典
    var data=this.qs.stringify({
        type:8,
