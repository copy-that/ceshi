        ...
        <div class="navlist">
            <div class="navlist_top">
                <div v-for="(item,key) in list" v-bind:key="key">
                    <ul v-if="item.valueField!=='zd_fenzhan'&&item.valueField!=='zd_zu_price'">
                    ...
                    <ul v-if="item.valueField=='zd_zu_price'">
                        <li class="title">{{item.valueName}}</li>
                        ...
                        <li class="sort" :class="res.id==label[key]?'active':''"  v-for="(res,index) in item.zdValue"  v-bind:key="index" @click="changeList(key,res.id)" :id="res.id">
                            ...
                            <!-- <span v-else>{{res.value}}元/天</span> -->
                            <span v-else-if="res.valueType==0">{{res.value}}元/天以下</span>
                            <span v-else-if="res.valueType==1">{{res.value}}元/天</span>
                            <span v-else-if="res.valueType==2">{{res.value}}元/天以上</span>
                        </li></div>
                    </ul>
            ...
            <div class="navlist_bot">
                ...
                <div class="infolist" v-for="(item,index) in infolist" v-bind:key="index">
                    ...
                    <ul class="money" style="width:260px;text-align: center"><span>{{item.price }}</span>元/天</ul></router-link>
                </div>
                <el-pagination
                    background
...
export default {
  ...
  methods: {
    ...
    changeList(key,id){
        ...
        this.$http('/app/rendInfoApi/getHotelList','post',data,'').then(
            res=>{
